...
public class MyPanel extends JPanel implements KeyListener , Runnable{
    ...
    //定义一个用于存放Node的Vector,用于回复敌人的坐标与方向
...
//
        ...
        if(file.exists()==false){
            key="1";//存档文件不存在，只能开始新游戏
            System.out.println("存档文件不存在，只能开始新游戏");
        }else {
            nodes=Recorder.getNodesEnemyTankRec();
        }

       // 将Mypanel对象的enemyTRanks 设置给Recorder的enemyTanks
        Recorder.setEnemyTanks(enemyTanks);

        hero = new Hero(100, 300);//初始化我的坦克
        ...
        //播放指定的音乐
        AePlayWave aePlayWave = new AePlayWave("C:\\Users\\33419\\Desktop\\ideatest\\src\\111.wav");
        aePlayWave.start();
